...
### Added
- **Remote execution mode (`src/remote/`)**: run training on rented compute with your own provider API key — Merlina provisions the instance, runs the job, streams progress home, and tears the instance down. Bring-your-own-RunPod-key (`RUNPOD_API_KEY` in `.env`); you pay the provider directly.
  - **Piecemeal stage pipeline**: a remote run is a plan of stages (`train` → `merge`) that hand off through a durable artifact store (private HF Hub repo per run by default), so each stage can run on a *different* machine — train on an 8-GPU pod, merge on a cheap big-disk box, or skip the merge entirely and ship the adapter (`remote.merge_strategy: auto|local|remote|skip`).
  - **MoE-aware sizing advisor** (`src/remote/sizing.py`): reads the model's real `config.json` (DeepSeek-V3-style MoE fields included), estimates VRAM/disk from actual architecture numbers, and picks the cheapest feasible GPU type/count from live provider offers. Correctly sizes 1T-class MoEs like Kimi-K2 (~570 GB 4-bit weights → multi-GPU model-parallel pick) where name-based estimates are nonsense. Preview via `POST /remote/plan` without provisioning anything; `GET /remote/offers` lists rentable GPUs with prices.
  - **Remote worker** (`src/remote/worker_entry.py`): instance-side entry point that reuses the existing training pipeline (`run_training_sync` / DDP) against a local job DB and serves progress over a token-authenticated HTTP endpoint the control plane polls — your machine needs no public address. Big models shard across the instance's GPUs via the existing `device_map=auto` path.
  - **Safety rails**: instances are terminated in a `finally` (opt-out via `remote.keep_instance_on_failure` for debugging), `remote.max_runtime_hours` hard-caps the spend, `remote.max_cost_per_hr` constrains the instance pick, stop requests forward to the worker for a graceful checkpoint before teardown.
  - Remote jobs run in their own queue (`REMOTE_MAX_CONCURRENT_JOBS`, default 2) so they never block local-GPU slots; pre-flight validation skips local GPU/VRAM/disk checks for remote jobs and validates provider key, Hub-reachable datasets, and Hub-hosted base models instead.
  - API-only for now (`TrainingConfig.remote` block); form UI is a follow-up. New deps: `requests` (explicit), `scripts/remote_worker_bootstrap.sh` for bare PyTorch pods, guide at `docs/user/remote-training.md`.
