...
- **Architecture-aware VRAM estimation** (`src/vram_estimator.py`): pre-flight's VRAM check no longer guesses from the model *name* with a linear formula — it reads the model's real `config.json` (local directory or HF Hub, cache/offline-aware, exact parameter counts from safetensors metadata) and models each memory component separately: quantization-aware weights (embeddings/lm_head stay bf16 under bitsandbytes), LoRA adapter size computed from the actual `target_modules` (+ full `modules_to_save` copies), optimizer states by `optimizer_type` (8-bit vs 32-bit vs SGD…), GQA/flash-vs-eager/gradient-checkpointing-aware activations, vocab-size-driven logits memory (the dominant term on 128k-vocab models — why "the same 8B" OOMs on Llama-3 but not Mistral), the 2× forward batch of paired preference modes (ORPO/DPO/…), and the frozen reference model DPO/KTO load when LoRA is off. Insufficient-VRAM errors now name the dominant component and rank suggestions by what would actually help (e.g. `gradient_checkpointing` when activations dominate, `use_liger` when logits do). The name-pattern fallback remains for models with no readable config. New `POST /estimate/vram` endpoint returns the full breakdown (no GPU required), and the training form's pre-submit dialog now shows the server-side breakdown instead of the old client-side guess (which remains as an offline fallback). Tests: `tests/test_vram_estimator.py`.
- **HuggingFace organization selection for uploads**: `TrainingConfig` (and both upload request models) now take an `hf_namespace` — the org or account a model is published under — and a new `POST /hf/namespaces` endpoint lists the namespaces a token can actually publish to (the account itself plus every org, flagging read-only memberships). The training form, the per-job upload modal, and the Export → Hub panel each get an "Upload To" dropdown with a **Check Orgs** button that fills it from the token. Pre-flight warns when the chosen namespace isn't one the token belongs to, or when your role there looks read-only. Tests: `tests/test_hf_namespaces.py`, `tests/test_hf_namespace_api.py`, plus the namespace blocks in `tests/frontend/test_form_config.mjs` and `tests/frontend/test_integration.spec.mjs`.
...
### Fixed
- **A missing W&B key killed the run instead of just the metrics**: pre-flight *warns* when `use_wandb` is set with no key available, promising "training will proceed but metrics won't be logged". The worker did not honour that — it set `log_with="wandb"` regardless, accelerate initialised wandb, and the job died with `UsageError: api_key not configured (no-tty)` **after** the model and dataset had loaded. Since `use_wandb` defaults to true, any machine without a key hit this on its first job. The worker now resolves a usable key first (consulting `wandb.api.api_key`, so `~/.netrc` and a prior `wandb login` count, not just the env var) and falls back to no tracker when there isn't one. Tests: `tests/test_wandb_missing_key.py`.
- **Memory guard stopped healthy training right after eval on unified-memory machines**: an eval pass leaves a multi-GB reserved-but-unallocated gap in the CUDA caching allocator (a real Spark run: 68.8 GB reserved vs 61.4 GB allocated), and on a shared pool that idle cache is host RAM the OS can't see — free RAM dipped under the watchdog's soft floor and the job was gracefully stopped at ~20% for pressure that was pure allocator slack. Two-part fix: (1) all three trainer paths (`training_runner`, `train_worker`, `training_runner_vlm`) now add an `EvalMemoryTeardownCallback` when the guard is active, releasing the eval cache back to the pool right after each evaluation; (2) the watchdog itself now reclaims the allocator cache and re-checks before requesting a stop at the soft floor — escalating only if free RAM stays under the floor, or if pressure returns within `MEMORY_GUARD_RECLAIM_COOLDOWN_SECONDS` (default 60) of the last reclaim. The hard floor still aborts immediately with no reclaim attempt. Tests: `tests/test_memory_guard.py`. the training worker's upload path passed the bare `output_name` straight to `upload_folder()`. `create_repo()` resolves a bare name against the token's account, but `upload_folder()` resolves nothing — so the repo was created (in the wrong namespace, or found in the right one) and the upload then failed with `RepositoryNotFoundError: 404 ... /api/models/<name>/preupload/main`, leaving the trained model only on local disk. Uploads now resolve the target namespace up front and use the fully-qualified `repo_id` that `create_repo()` returns for every subsequent call (folder upload, README, config image, upload-state record). An `output_name` that already contains an explicit `org/name` still wins over the selected namespace.
- **A deleted job could still be trained**: deleting a job removed its database record but left it sitting in the queue, so a worker would later pick it up and start a full training run for a job the user had already thrown away — writing status updates to a record that no longer existed. The queue worker now verifies the job record still exists (and that the entry wasn't dropped) before executing anything, and `DELETE /jobs/{id}` clears the job's queue entry, releasing the stored config (which can hold an `hf_token`) instead of keeping it alive in a stale queue item. `DELETE /jobs` does the same for every queued job, so "Clear All Jobs" no longer leaves phantom entries in `/queue/status`. Both responses now report `removed_from_queue`. Tests: `tests/test_queue.py`, `tests/test_api_comprehensive.py`.
...
- **API restart orphaned the training worker and deleted its progress file**: worker run files (config + progress JSONL) now live under `data/worker_runs/<job_id>/` instead of a `tempfile.mkdtemp()` directory (which systemd PrivateTmp / tmp cleaners could delete under a live worker), and the worker's pid + progress-file path are persisted in the jobs DB. On startup the server reconciles: still-alive workers are re-adopted by a monitor (`src/worker_reattach.py`) that keeps tailing progress, forwards `/stop` requests (SIGTERM→SIGKILL escalation), and finalizes the job when the worker exits; jobs whose worker is gone are marked `failed` with a retry hint instead of presenting as "training" forever. `/jobs/{id}/stop` now falls back to setting the DB stop flag when the in-memory queue has lost track of a job. Cleanup no longer deletes run files while the worker is still alive.
- **Worker error handler could itself raise, destroying the original exception**: every reporting step in `train_worker.py`'s failure/finalization paths (job-DB update, progress-file write, `wandb.finish`, memory-guard shutdown, completion record) is now guarded independently, so a secondary failure (locked DB, vanished progress dir) can't mask the real error or leave the job stuck in a non-terminal status.
- **`GET /jobs/{id}/config` returned `hf_token` and `wandb_key` in plaintext**: the endpoint now strips credential fields (via the same `strip_secrets` rule used for saved configs and model cards) before responding. The stored config keeps them so `/jobs/{id}/retry` still works.
- **Hardcoded default dataset**: `DatasetConfig.source` no longer defaults to `schneewolflabs/Athanorlite-DPO`, so a request missing its dataset config fails validation/pre-flight instead of silently training on a real repo. The MCP `start_training` tool's `dataset_repo_id` is now a required argument for the same reason.
...
### Added
- **Unified-memory protection for DGX Spark / Grace-Blackwell** (`src/memory_guard.py`): on machines where GPU and system RAM share one pool (GB10 "DGX Spark", GH200/GB200, Jetson), a training memory spike used to freeze or crash the entire machine instead of raising a catchable CUDA OOM. Merlina now auto-detects unified-memory systems (by GPU name or by the GPU claiming ≥90% of system RAM; override with `UNIFIED_MEMORY=true/false`) and, for every training job: caps the CUDA allocator via `set_per_process_memory_fraction` so spikes fail as ordinary OOMs with a reserve kept for the OS (`MEMORY_GUARD_RESERVE_GB`, default 12 GB); runs a watchdog that gracefully stops training (checkpoint saved) when free system RAM drops below a soft floor (`MEMORY_GUARD_SOFT_FREE_GB`, default 8 GB) and aborts the job below a hard floor (`MEMORY_GUARD_HARD_FREE_GB`, default 3 GB); and appends an fsync'd memory timeline to `data/memory_guard.log` so a post-reboot investigation has the "last lines before death" that a hard crash never leaves behind. Wired into the text, VLM, and diffusion runners plus the DDP subprocess worker; pre-flight checks now report unified-memory systems and budget VRAM against actually-free shared-pool memory (minus the reserve) instead of the raw pool size. Everything is inert on discrete-GPU machines. Tests: `tests/test_memory_guard.py`.
- **HuggingFace dataset subset/config selection**: dataset sources now accept an optional `config_name` (HuggingFace's `load_dataset(name=...)` argument), so you can pick a specific configuration/subset of a multi-config dataset (e.g. the `high_quality` config of `schneewolflabs/Alembic-DPO`). Threaded through `DatasetSource`, both HF loaders (regular + streaming), and the loader factory; surfaced as a "Subset / Config" field on each dataset card and shown in the reproducibility model card. When left blank, the dataset's default configuration loads exactly as before. Tests: `tests/test_dataset_config_name.py` and the `config_name` round-trip case in `tests/frontend/test_form_config.mjs`.
- **MCP server** (`mcp_server.py`, `merlina-mcp` command): exposes the Merlina REST API as [Model Context Protocol](https://modelcontextprotocol.io) tools so an LLM agent (Claude Desktop, Claude Code, Cursor, …) can queue training jobs, monitor progress, preview datasets, and manage the GPU queue conversationally. Tools include `start_training`, `get_job_status`/`get_job_history`/`get_job_metrics`, `stop_job`, `queue_status`, `validate_training_config`, `preview_dataset`, `list_local_models`, `list_gpus`, and `get_stats`. It's a thin HTTP client to a running Merlina server (no torch import), configured via `MERLINA_API_URL` (default `http://localhost:8000`). New optional extra `merlina[mcp]` (adds `mcp[cli]` + `httpx`), docs at `docs/user/mcp.md`, and `tests/test_mcp_server.py`.
