...
### Fixed
- **Hub uploads ignored the organization and 404'd**: the training worker's upload path passed the bare `output_name` straight to `upload_folder()`. `create_repo()` resolves a bare name against the token's account, but `upload_folder()` resolves nothing — so the repo was created (in the wrong namespace, or found in the right one) and the upload then failed with `RepositoryNotFoundError: 404 ... /api/models/<name>/preupload/main`, leaving the trained model only on local disk. Uploads now resolve the target namespace up front and use the fully-qualified `repo_id` that `create_repo()` returns for every subsequent call (folder upload, README, config image, upload-state record). An `output_name` that already contains an explicit `org/name` still wins over the selected namespace.
