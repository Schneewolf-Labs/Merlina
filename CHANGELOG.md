...
### Added
- **Separate evaluation dataset source (UI)**: the dataset step now exposes an "Evaluation Set" selector — choose *Split from training data* (the existing random `test_size` hold-out) or *Use a separate eval dataset*. Selecting the latter reveals a dedicated dataset card (same source picker and column mapping as a training dataset) that maps to `DatasetConfig.eval_source`; when set, the full training data is used for training and the random split is skipped. The mode disables the Test Split Size control to make the relationship clear, and round-trips through config save/load. Backend support for `eval_source` already existed — this wires it into the frontend. Covered by new frontend integration, form-config, and validation tests.
