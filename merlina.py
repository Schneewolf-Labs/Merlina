...
def _warm_training_runner_import():
    ...
    src.training_runner is imported lazily by _make_training_callback; without
    this warm-up the first /train after a server start pays the full import
    cost inside the request, which can stall the response long enough for
    clients/proxies to give up — after the job row already exists.
...
async def create_training_job(config: TrainingConfig, priority: Optional[str] = "normal"):
    ...
    # Build the callback BEFORE creating the job record, off the event loop:
    # the first call after server start may still need to import the heavy
    # training stack, and doing that on the loop (or after the job exists)
    # can stall this response until the client gives up — leaving a created
    # job with a seemingly failed request.
    training_callback = await asyncio.to_thread(_make_training_callback, loop)
...
async def retry_job(job_id: str, priority: Optional[str] = "normal"):
    ...
    # As in /train: build the callback (which may lazily import the training
    # stack) off the event loop and before the job record exists.
    training_callback = await asyncio.to_thread(_make_training_callback, loop)
