...
class DatasetConfig(BaseModel):
    ...
    training_mode: str = Field("orpo", description="Training mode: 'sft', 'orpo', 'dpo', 'simpo', 'cpo', 'ipo', 'kto', or 'grpo'. For SFT/KTO/GRPO, rejected column is optional.")
...
class TrainingConfig(BaseModel):
    ...
    training_mode: str = Field(
        "orpo",
        description="Training mode: 'sft', 'orpo', 'dpo', 'simpo', 'cpo', 'ipo', 'kto', or 'grpo'"
    )

    # Preference optimization parameters
    beta: float = Field(0.1, ge=0.01, le=10.0, description="Beta parameter for preference optimization (ORPO, DPO, SimPO, CPO, IPO, KTO) or KL penalty weight (GRPO)")
    ...
    grpo_num_generations: int = Field(4, ge=2, le=16, description="Number of completions to generate per prompt (GRPO)")
